...
interface RetranscribePageArgs {
  book_name: string;
  page_number: number;
  model?: string;
}
...
export async function retranscribePage(
  ...
  if (!imageData) {
    throw new Error(
      `Page ${page_number} has no associated image. It may have been manually inserted and has no scan.`
    );
  }

  process.stderr.write(
    `[OCR MCP] Re-transcribing "${book_name}" page ${page_number} with ${model}...\n`
  );
