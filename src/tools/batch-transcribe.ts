...
interface BatchTranscribeArgs {
  ...
  overwrite?: boolean;   // reprocess already-complete books
  dry_run?: boolean;     // preview what would be submitted without doing it
}

export async function batchTranscribe(args: BatchTranscribeArgs): Promise<string> {
  ...
  const inProgressBookIds = new Set<number>(
    inProgressJobs.flatMap((job) => JSON.parse(job.book_ids) as number[])
  );
  ...
  if (book_names.length > 0) {
    candidates = driveFiles.filter((f) =>
      book_names.some(
        (name) =>
          f.name === name ||
  ...
  if (toQueue.length === 0) {
    ...
    if (skippedInProgress.length > 0) reasons.push(`${skippedInProgress.length} already in an active batch`);
    if (skippedComplete.length > 0) reasons.push(`${skippedComplete.length} already complete`);
  ...
  lines.push(
    ...
    `Batch ID: ${batchId}`,
    ``,
    `Queued (${batchRequests.length}):`,
    ...batchRequests.map((r) => `  • ${r.bookTitle}`),
  );
