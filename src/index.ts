...
async function processBatchByKind(batchId: string): Promise<{ summary: string }> {
...
server.tool(
  'check_batch',
  'Polls an Anthropic batch job for status. If complete, processes all results and stores them — transcriptions for an OCR batch, or sentiment scores for a score_pages batch (routed automatically by batch kind).',
  {
...
const toSections = (
  raw: Array<{ start_tag?: string; end_tag?: string; name?: string }> | undefined,
) =>
  (raw ?? [])
...
server.tool(
  'list_tags',
  'Lists all distinct page tags used across the library — the values you can group or filter sentiment charts by with chart_sentiment.',
  {},
  async () => {
    try {
...
server.tool(
  'list_methods',
  'Lists all scoring methods (instruments) — the built-in Claude default plus any you created. Use these names with score_pages and chart_sentiment.',
  {},
  async () => {
    try {
...
server.tool(
  'delete_method',
  'Deletes a scoring method and all sentiment scores it produced. This cannot be undone.',
  {
