 ...
 */
export function imageRenderScale(): number {
  if (!blob.isConfigured()) return 1.0;
...
export async function readPageImageBytes(
  bookId: number,
  pageNumber: number,
...
export async function readPageImageBase64(bookId: number, pageNumber: number): Promise<string | null> {
...
export async function writePageImageBytes(
  bookId: number,
  pageNumber: number,
  bytes: Buffer,
  contentType?: string,
...
export async function writePageImageBase64(bookId: number, pageNumber: number, base64: string): Promise<void> {
