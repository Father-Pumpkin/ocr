...
export interface MatchedTerm {
  term: string;
  /** The dictionary's normalized 0–1 value for this term. */
  value: number;
  /** Value after negation flipped it, when that applies. */
  effective: number;
  negated: boolean;
  /** Token positions where it occurred, so repeats are visible. */
  positions: number[];
}
...
export interface ScoreExplanation {
  ...
  methodKind: string;
  dimension: string;
  /** The score stored for this (page, dimension, method), if there is one. */
  storedScore: number | null;
  /** Recomputed from the text and the dictionary — should match the stored one. */
  recomputedScore: number | null;
  /** Every dictionary hit, most influential first. */
  matched: MatchedTerm[];
  tokenCount: number;
  matchCount: number;
  /** The page text, so the words can be read in context. */
  text: string;
  /** LLM methods: the rationale the model gave. */
  rationale: string | null;
  /** Set when there is nothing to explain, with the reason. */
  note: string | null;
}
...
export async function explainPageScore(opts: {
  ...
  if (!book) throw new ExplainError(`No book called "${opts.book}".`);

  const pages = await getPages(book.id, opts.pageNumber, opts.pageNumber);
  const page = pages[0];
  if (!page) throw new ExplainError(`${opts.book} has no page ${opts.pageNumber}.`);

  const method = await getMethodByName(opts.method);
  if (!method) throw new ExplainError(`No scoring method called "${opts.method}".`);

  const dimension = await getDimensionByName(opts.dimension);
  if (!dimension) throw new ExplainError(`No dimension called "${opts.dimension}".`);

  const stored = (await getPageSentiment(page.id)).find(
    (r) => r.dimension_id === dimension.id && r.method_id === method.id,
  );
  ...
  const byTerm = new Map<string, MatchedTerm>();
  let sum = 0;
  let matchCount = 0;
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    else byTerm.set(tokens[i], { term: tokens[i], value, effective, negated, positions: [i] });
  }
  ...
  const matched = [...byTerm.values()].sort(
    (a, b) =>
      Math.abs(b.effective - 0.5) * b.positions.length - Math.abs(a.effective - 0.5) * a.positions.length,
  );
