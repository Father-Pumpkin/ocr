...
import { importLexicon, previewLexicon, decodeLexiconBytes, type LexiconImportResult } from './lexicon-import.js';
import { getLexiconByName, getAllMethods, createMethod, type MethodRow } from './database.js';
...
export const POLARITY_DIMENSION_DESCRIPTION =
  'Word-level sentiment polarity, measured by dictionary lookup. High = positive words dominate the page; ' +
  'low = negative words dominate. Scored by lexicon methods rather than by reading for meaning.';

/** How a preset's published files are laid out, so an import can be pre-filled. */
export interface LexiconFormatSpec {
  /** 'delimited' = a table with values; 'wordlist' = one term per line, polarity by file. */
  kind: 'delimited' | 'wordlist';
  delimiter?: string;
  termColumn?: string;
  valueColumn?: string;
  /**
   * Whether the published file has a header row. Several of these don't — SO-CAL
   * and Linguakit are bare two-column files — and guessing wrong eats a term and
   * names the columns after it, so it is stated rather than detected.
   */
  hasHeader?: boolean;
  /**
   * For files whose value column holds words rather than numbers, e.g.
   * Linguakit's POSITIVE / NEGATIVE.
   */
  labelValues?: Record<string, number>;
  /** The dictionary's native value range, normalized to 0–1 on import. */
  scale: [number, number];
  /**
   * Word lists only: a filename fragment and the value every term in that file
   * takes. A positive file and a negative file import under one lexicon name.
   */
...
export interface LexiconPreset {
  id: string;
  label: string;
  /** Substrings matched (loosely) against a lexicon or file name. */
  aliases: string[];
  description: string;
  /**
   * Where to obtain it. `confirmed: false` means I could not verify a canonical
   * distribution — the entry is still usable via upload, but don't trust a URL
   * that isn't there.
   */
  source: { url: string | null; licence: string | null; confirmed: boolean };
  /** null when the published layout isn't known well enough to pre-fill. */
  format: LexiconFormatSpec | null;
  /**
   * True when the layout above was read off the actual published file rather
   * than inferred from how such lexicons are usually shaped.
   */
  formatVerified: boolean;
  /** Exact files to load, when the distribution has more than one candidate. */
  files?: string[];
  /** Anything the user needs to know before loading it. */
  note: string;
}
...
export const LEXICON_PRESETS: LexiconPreset[] = [
  {
    ...
    note:
      "Upstream AFINN has no Spanish list, so this is the Spanish translation used in the R/tidytext " +
  ...
  },
  {
    ...
    description:
      "Simon Fraser University's Semantic Orientation CALculator dictionaries, published per part of speech.",
    source: {
    ...
    note:
      'Headerless `word<sep>score` files, −5..5, one per part of speech. Two traps in this distribution. ' +
  ...
  },
  {
    ...
    note:
      'iSOL is a polarity lexicon: two plain word lists, no scores — 2,509 positive and 5,626 negative, ' +
  ...
  },
  {
    ...
    note:
      'The Spanish lexicon is sentiment/es/lex_es inside the toolkit — one file, `word<TAB>POSITIVE` or ' +
  ...
  },
  {
    ...
    description:
      'Chen & Skiena\u2019s multilingual polarity lexicons — the Spanish half of a set covering 136 languages.',
    source: {
    ...
    note:
      'SBU is Chen & Skiena, "Building Sentiment Lexicons for All Major Languages" (ACL 2014), from Stony ' +
...
export function presetFor(name: string): LexiconPreset | undefined {
  const n = name.toLowerCase().replace(/[^a-z0-9]/g, '');
  return LEXICON_PRESETS.find((p) =>
    p.aliases.some((a) => n.includes(a.toLowerCase().replace(/[^a-z0-9]/g, ''))),
  );
}

/** Stable method name for a lexicon, so repeat runs reuse one row. */
export function lexiconMethodName(lexiconName: string): string {
  return `lex-${lexiconName}`;
}

/** Register the scoring method for a lexicon, or return the one already bound. */
export async function ensureLexiconMethod(
  lexiconId: number,
  lexiconName: string,
  negation = true,
): Promise<MethodRow> {
  ...
  return createMethod(
    lexiconMethodName(lexiconName),
    'lexicon',
    JSON.stringify({ lexicon_id: lexiconId, aggregation: 'mean', negation }),
  );
}
...
/**
 * A sidecar `<file>.mapping.json` overrides everything inferred, for a
 * dictionary whose layout no preset covers:
 *   { "lexicon": "my-lex", "termColumn": "word", "valueColumns": {"score": "polarity"},
 *     "scaleMin": -5, "scaleMax": 5, "delimiter": "\t" }
 * or, for a word list:
 *   { "lexicon": "my-lex", "dimension": "polarity", "fixedValue": 1,
...
interface SidecarMapping {
  ...
  scaleMax?: number;
  delimiter?: string;
  /** Force the header decision for a file whose first row is ambiguous. */
  hasHeader?: boolean;
...
function readSidecar(filePath: string): SidecarMapping | null {
  const sidecar = `${filePath}.mapping.json`;
  if (!fs.existsSync(sidecar)) return null;
  try {
    return JSON.parse(fs.readFileSync(sidecar, 'utf8')) as SidecarMapping;
  } catch {
    return null;
  }
}
...
function planImport(
  filePath: string,
  lexiconName: string,
): (Parameters<typeof importLexicon>[0] & { negation: boolean }) | null {
  ...
  if (!termColumn || !valueColumn) return null;
  ...
  if (needsLabels && !spec?.labelValues) return null;
 ...
 */
function lexiconNameFor(filePath: string, root: string): string {
 ...
 */
export async function seedLexiconsFromDisk(): Promise<SeedOutcome[]> {
  ...
  for (const [lexiconName, group] of byLexicon) {
    ...
    let imported: LexiconImportResult | null = null;
    for (const file of group) {
      ...
      try {
        ...
        if (!plan) {
          outcomes.push({
            ...
            status: 'failed',
            reason:
              'Could not work out its layout — add a <file>.mapping.json beside it. ' +
