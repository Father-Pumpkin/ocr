 ...
 */
import { getLexiconTerms, type DimensionRow, type MethodRow } from './database.js';
import { scoreTranscription, DEFAULT_MODEL } from './ocr.js';

export interface ScoreResult {
  score: number; // 0–1
  rationale: string;
  /** Provenance stored in page_sentiment.model — an LLM model id or "lexicon:<name>". */
  model: string;
}

export interface Scorer {
  /** Score one page on a dimension. null = no signal (don't store). */
  score(pageText: string, dimension: DimensionRow): Promise<ScoreResult | null>;
}
...
export function parseMethodConfig(method: MethodRow): MethodConfig {
  try {
    return JSON.parse(method.config || '{}') as MethodConfig;
  } catch {
    return {};
  }
}
...
class LlmScorer implements Scorer {
  constructor(private readonly model: string, private readonly prompt?: string) {}

  async score(pageText: string, dimension: DimensionRow): Promise<ScoreResult | null> {
...
class LexiconScorer implements Scorer {
  // Lazily loaded per dimension: term → normalized 0–1 value.
  private readonly cache = new Map<number, Map<string, number>>();

  constructor(
    private readonly lexiconId: number,
    private readonly negation: boolean,
    private readonly label: string,
  ) {}

  private async termsFor(dimensionId: number): Promise<Map<string, number>> {
  ...
  async score(pageText: string, dimension: DimensionRow): Promise<ScoreResult | null> {
    ...
    if (terms.size === 0) return null; // lexicon doesn't cover this dimension
    ...
    if (matched === 0) return null; // no lexicon hits on this page → no signal
 ...
 */
export async function getScorer(method: MethodRow, modelOverride?: string): Promise<Scorer> {
