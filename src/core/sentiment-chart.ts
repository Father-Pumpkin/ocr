...
export function renderSentimentChartPng(result: AnalyzeResult): string {
  const W = 920;
  const H = 520;
  ...
  const yOf = (s: number): number => bottom - Math.max(0, Math.min(1, s)) * plotH;
  ...
  function drawSeries(): void {
    ...
    for (const g of groups) {
      for (const p of g.points ?? []) {
        if (p.page_number < minX) minX = p.page_number;
        if (p.page_number > maxX) maxX = p.page_number;
      }
    }
    ...
    const xOf = (pn: number): number => left + ((pn - minX) / (maxX - minX)) * plotW;
  ...
  function drawLegend(): void {
    ...
    groups.slice(0, 18).forEach((g, gi) => {
      ...
      if (multiMethod) parts.push(truncate(g.method, 14));
      if (multiDim) parts.push(truncate(g.dimension, 12));
...
function chartTitle(result: AnalyzeResult): string {
