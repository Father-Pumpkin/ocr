...
function runRequestFromBody(body) {
    const b = (body ?? {});
    const style = str(b.style);
    if (!style)
        throw new AnalysisInputError('Choose an analysis style.');
    const arr = (v) => Array.isArray(v) && v.length ? v.map(String) : undefined;
    ...
    return {
        style,
        mode: rawMode ? rawMode : undefined,
        books: arr(b.books),
...
analysisRouter.post('/analysis/dimensions', requireMember, async (req, res) => {
    try {
        const b = (req.body ?? {});
        const dimension = await createDimensionData(str(b.name), str(b.description), str(b.minLabel) || 'Low', str(b.maxLabel) || 'High');
...
analysisRouter.post('/analysis/lexicons/preview', requireMember, (req, res) => {
    try {
        ...
        if (!content.trim())
            throw new AnalysisInputError('The uploaded file is empty.');
        res.json(inspectLexicon({
            content,
...
analysisRouter.post('/analysis/lexicons', requireMember, async (req, res) => {
    try {
        ...
        const result = await uploadLexicon({
            ...
            valueColumns: valueColumns && typeof valueColumns === 'object' && !Array.isArray(valueColumns)
                ? Object.fromEntries(Object.entries(valueColumns).map(([k, v]) => [k, String(v)]))
                : {},
            fixedValue,
            ...
            delimiter: typeof b.delimiter === 'string' && b.delimiter ? b.delimiter : undefined,
            hasHeader: typeof b.hasHeader === 'boolean' ? b.hasHeader : undefined,
            labelValues: b.labelValues && typeof b.labelValues === 'object' && !Array.isArray(b.labelValues)
                ? Object.fromEntries(Object.entries(b.labelValues)
                    .map(([k, v]) => [k, Number(v)])
