 ...
 */
export function decodeLexiconBytes(bytes) {
...
function defaultDelimiter(fileName) {
...
function readSource(input) {
    if (typeof input.content === 'string') {
        if (!input.content.trim())
            throw new Error('The uploaded lexicon file is empty.');
...
export function previewLexicon(input) {
    ...
    if (format === 'json') {
        ...
        labelColumns = columns
            ...
            if (values.length === 0 || values.some(isNumeric))
                return null;
    ...
    const numericColumns = columns.filter((c) => sampleRows.length > 0 && sampleRows.every((r) => r[c] !== '' && !Number.isNaN(Number(r[c]))));
    ...
    const values = allRows.length
        ? allRows.flatMap((r) => numericColumns
...
function isWordListLines(lines, delimiter) {
...
function syntheticColumns(width) {
...
export async function importLexicon(input) {
    ...
    const labelMap = new Map(Object.entries(input.labelValues ?? {}).map(([k, v]) => [k.trim().toUpperCase(), v]));
    ...
    if (wordListMode) {
        ...
        const headerless = input.hasHeader === undefined ? !detectHasHeader(parsed.slice(0, 40)) : !input.hasHeader;
        ...
        const valueCols = Object.entries(valueColumns).map(([col, dim]) => {
            ...
            if (idx < 0)
                throw new Error(`Value column "${col}" not found in ${where}`);
            return { idx, dim };
        });
