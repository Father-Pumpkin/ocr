...
export async function getAnalysisOptions(opts = {}) {
    ...
    const visibleDimensions = opts.role === 'guest' ? dimensions.filter((d) => GUEST_DIMENSIONS.includes(d.name)) : dimensions;
    return {
        styles,
...
export async function resolveMethodForRun(req) {
    ...
    if (!m)
        throw new AnalysisInputError('Could not resolve a scoring method for this run.');
    return m;
}
/**
 ...
 */
async function resolveMethod(req, create) {
    ...
    if (prefix === 'lexicon') {
        ...
        if (!create)
            return (await getMethodByName(lexiconMethodName(lex.name))) ?? null;
...
export async function saveRubricMethod(opts) {
    ...
    if (!name)
        throw new AnalysisInputError('Give the rubric a name so it can be reused and compared later.');
    if (!rubric)
        throw new AnalysisInputError('The rubric cannot be empty.');
...
export async function exportResults(input, format, opts = {}) {
    ...
    const rows = opts.includeRationale === false ? result.rows.map((r) => ({ ...r, rationale: null })) : result.rows;
