export const EXPORT_FORMATS = ['pages.csv', 'summary.csv', 'json'];
/**
 ...
 */
function exportFilename(result, format) {
    const scope = result.books.length === 1 ? result.books[0] : `${result.books.length}-books`;
...
export function buildExport({ result, rows, format }) {
