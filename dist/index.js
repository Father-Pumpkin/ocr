...
// ---- Tool: check_batch ------------------------------------------------------
server.tool('check_batch', 'Polls an Anthropic batch job for status. If complete, processes all results and stores transcriptions in the database.', {
...
// ---- Tool: update_page -------------------------------------------------------
server.tool('update_page', 'Manually corrects the transcription for a specific page. Sets is_edited=true in the database.', {
...
// ---- Tool: tag_page ---------------------------------------------------------
server.tool('tag_page', 'Sets narrative tags on a specific page (e.g. "inciting incident", "climax"). Replaces all existing tags with the supplied list.', {
    book_name: z.string().describe('The book filename or title.'),
...
// ---- Tool: get_page_image ---------------------------------------------------
server.tool('get_page_image', 'Returns a rendered image of a specific book page as base64 JPEG. Caches locally so Drive is only hit once per book.', {
    book_name: z.string().describe('The book filename or title.'),
    page_number: z.number().int().positive().describe('The 1-based page number.'),
...
// ---- Tool: set_page_image ---------------------------------------------------
server.tool('set_page_image', 'Uploads a custom image for a specific page, replacing any existing image. Useful for manually inserted pages or rescanned pages that need a new image.', {
    book_name: z.string().describe('The book filename or title.'),
    page_number: z.number().int().positive().describe('The 1-based page number.'),
...
// ---- Tool: insert_page ------------------------------------------------------
server.tool('insert_page', 'Inserts a new blank page after the specified page number, renumbering all subsequent pages.', {
    book_name: z.string().describe('The book filename or title.'),
    after_page_number: z
        .number()
        .int()
...
// ---- Tool: delete_page ------------------------------------------------------
server.tool('delete_page', 'Deletes a page from a book and renumbers all subsequent pages. Also removes any cached image for that page.', {
    book_name: z.string().describe('The book filename or title.'),
...
registerAppTool(server, 'view_transcriptions', {
...
registerAppResource(server, 'transcription-viewer', VIEWER_RESOURCE_URI, { mimeType: RESOURCE_MIME_TYPE }, async () => {
