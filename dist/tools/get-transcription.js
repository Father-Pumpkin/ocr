...
export async function getTranscription(args) {
    ...
    if (!book) {
        return (`Book not found: "${book_name}"\n` +
    ...
    if (pages.length === 0) {
        const rangeText = page_start !== undefined || page_end !== undefined
    ...
    const rangeLabel = page_start !== undefined || page_end !== undefined
    ...
    if (include_illustrations) {
        ...
        if (filteredPages.length === 0) {
            lines.push('(All pages in this range are illustration-only. Use include_illustrations=true to see them.)');
        }
    }
    return lines.join('\n');
}
